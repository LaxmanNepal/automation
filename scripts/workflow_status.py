            ...
            try:
                ...
                if latest and latest.get("conclusion") not in (None,"success"):
                    try:
                        ...
                        if failed:
                            failed_stage=failed[0].get("name")
                    except Exception:
                        failed_stage="Job details unavailable"
                status="healthy" if latest and latest.get("conclusion")=="success" else ("warning" if latest and latest.get("status") in ("in_progress","queued") or latest and latest.get("conclusion") else "waiting")
                rows.append({
                    "name":name,"workflow":file,"status":status,
