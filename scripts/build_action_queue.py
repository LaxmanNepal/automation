...
youtube_path = ROOT / "data" / "youtube" / "latest.json"
...
youtube = json.loads(youtube_path.read_text(encoding="utf-8")) if youtube_path.exists() else {"topics": []}
actions = []

for problem in problems.get("problems", []):
    if problem.get("kind") == "scan_warning" or problem.get("priority", 0) < 45:
        continue
    actions.append({"priority": problem["priority"], "category": "problem", "repository": problem.get("repository"),
                    "action": problem["title"], "reason": problem.get("detail", "Detected by Problem Detector.")})

for item in health.get("repositories", []):
    repo = item["repository"]
    if not item.get("accessible"):
        actions.append({"priority": 10, "category": "github", "repository": repo,
                        "action": f"Restore GitHub API access for {repo}", "reason": item.get("error", "unknown error")})
    elif item.get("archived"):
        actions.append({"priority": 8, "category": "github", "repository": repo,
                        "action": f"Review archived status of {repo}", "reason": "Repository is archived."})
    elif (item.get("open_issues") or 0) > 0:
        actions.append({"priority": 6, "category": "github", "repository": repo,
                        "action": f"Review open issues in {repo}", "reason": f"{item.get('open_issues')} open issue(s)."})

# Promote the best generated YouTube candidate into the execution queue.
for topic in youtube.get("topics", [])[:2]:
    actions.append({"priority": 9, "category": "youtube", "repository": None,
                    "action": f"Validate and produce: {topic.get('topic', '')}",
                    "reason": f"{topic.get('channel', '')} channel; intent={topic.get('intent', '')}; score={topic.get('score', 0)}."})

actions.append({"priority": 10, "category": "money", "repository": None,
