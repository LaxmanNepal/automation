...
def derive_resolution(evidence, previous):
...
def main():
    ...
    if remediation is None or verification is None:
        ...
        for rem in remediation.get("items", []):
            ...
            old_resolution = previous.get("resolution_state")
            resolution = derive_resolution(evidence, old_resolution)
            history = list(previous.get("history", []))
            prior_signature = history[-1].get("signature") if history else None
            signature = evidence + "|" + resolution + "|" + str((vrow or {}).get("verification_run_id"))
            if signature != prior_signature:
                history.append({"at":now,"evidence_state":evidence,"resolution_state":resolution,"signature":signature,"evidence_run_id":(vrow or {}).get("verification_run_id"),"evidence_url":(vrow or {}).get("verification_run_url"),"human_confirmation":False})
            items.append({"id":item_id,"remediation_id":rem.get("id"),"workflow":rem.get("workflow"),"stage":rem.get("stage"),"priority":rem.get("priority"),"evidence_state":evidence,"resolution_state":resolution,"human_confirmation_required":True,"human_confirmed":False,"causal_claim":"unverified","evidence_run_id":(vrow or {}).get("verification_run_id"),"evidence_url":(vrow or {}).get("verification_run_url"),"history":history})
        states = {x["resolution_state"] for x in items}
        status = "warning" if "blocked" in states else ("healthy" if items else "waiting")
        data = {"version":1,"generated_at":now,"status":status,"items":items,"coverage":"Persistent remediation state built from retained verification evidence.","human_validation_required":True,"resolution_boundary":"Automation never declares a real-world issue resolved. Human confirmation is required."}
    ...
    for item in data.get("items", []):
        lines += ["## " + str(item.get("workflow", "Workflow")),"- Remediation: " + str(item.get("remediation_id")),"- Evidence state: " + str(item.get("evidence_state")),"- Resolution state: " + str(item.get("resolution_state")),"- Human confirmed: " + str(item.get("human_confirmed")),"- Evidence run: " + str(item.get("evidence_run_id")),""]
